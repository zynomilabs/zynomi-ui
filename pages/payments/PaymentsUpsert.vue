<template>
  <section v-if="isUpsertPaymentsVisible" class="fixed inset-0 overflow-hidden" aria-labelledby="slide-over-title" role="dialog" aria-modal="true">
            ...
            <div class="flex-1 h-0 overflow-y-auto">
              <header class="space-y-1 py-6 px-4 bg-gray-200 sm:px-6">
                <div class="flex items-center justify-between space-x-3">
                  <h2 class="text-lg leading-7 font-medium text-gray-700">Payments</h2>
                  <div class="h-7 flex items-center">
                    <button v-on:click="closePaymentsPanel()" aria-label="Close panel" class="text-gray-600 hover:text-gray-900 transition ease-in-out duration-150">
                ...
                <div>
                  <p class="text-sm leading-5 text-gray-500">Create or update Payments record</p>
                </div>
              </header>
              <div class="flex-1 flex flex-col justify-between">
                <div class="px-4 divide-y divide-gray-200 sm:px-6">
                  <div class="space-y-6 pt-6 pb-5">
                    <form name="frmPayments" id="frmPayments" class="space-y-8 divide-y divide-gray-200" @submit.prevent="SaveAndUpdate">
                      <div class="sm:overflow-hidden">
                        <div class="bg-white space-y-6">
                          <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                            <div class="sm:col-span-6">
                              <label for="name" class="block text-sm font-medium text-gray-700"> Payment Ref. # </label>
                              <div class="mt-1 rounded-md shadow-sm">
                                <input v-model="data.invoice_nuber" type="text" id="invoice_nuber" name="invoice_nuber" autocomplete="invoice_nuber" class="flex-1 focus:ring-green-500 focus:border-green-500 block w-full min-w-0 sm:text-sm border-gray-300" />
                              </div>
                              <p class="mt-2 text-sm text-gray-500">This could be Invoice #., Purchase Order #., Bill #., Transcation #. etc.</p>
                            </div>

                            <div class="sm:col-span-6">
                              <Dropdownlist v-model="data.payment_to_user" @selected_item="handleSelectedIn_payment_to_user" id="payment_to_user" name="payment_to_user" label="Payment to" api="users" :selected_value="data.payment_to_user" />
                            </div>

                            <div class="sm:col-span-6">
                              <label for="name" class="block text-sm font-medium text-gray-700"> Payment date </label>
                              <div class="mt-1 rounded-md shadow-sm">
                            ...
                            <div class="sm:col-span-6">
                              ...
                              <div class="mt-1 rounded-md shadow-sm">
                                <input v-model="data.payment_amount" type="text" id="payment_amount" name="payment_amount" autocomplete="payment_amount" class="flex-1 focus:ring-green-500 focus:border-green-500 block w-full min-w-0 sm:text-sm border-gray-300" />
                              </div>
                            </div>

                            <div class="sm:col-span-6">
                              <Dropdownlist v-model="data.payment_currency" @selected_item="handleSelectedInPayment_currency" id="payment_currency" name="payment_currency" label="Currency" api="currency" :selected_value="data.payment_currency" />
                            </div>

                            <div class="sm:col-span-6">
                              <label for="name" class="block text-sm font-medium text-gray-700"> Exchange Rate </label>
                              <div class="mt-1 rounded-md shadow-sm">
                                <input v-model="data.currency_exchange_rate" type="text" id="currency_exchange_rate" name="currency_exchange_rate" autocomplete="currency_exchange_rate" class="flex-1 focus:ring-green-500 focus:border-green-500 block w-full min-w-0 sm:text-sm border-gray-300" />
                              </div>
                            </div>
  ...
  methods: {
    async upsertPayments(args) {
      try {
        if (this.$_.isUndefined(this.data.id)) {
          const { data } = await this.$axios.post(this.$config.apiURL + 'Payments', this.data);
        } else {
          const { data } = await this.$axios.put(this.$config.apiURL + 'Payments/' + this.data.id, this.data);
        }
        this.data = {};
    ...
    async deletePayments() {
      try {
        ...
        //end
        const { data } = await this.$axios.delete(this.$config.apiURL + 'Payments/' + this.data.id);
        $nuxt.$emit('evtRefreshPaymentsDatatable');
