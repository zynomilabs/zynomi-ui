<template>
  <div>
    <form name="frmSearchPayments" id="frmSearchPayments" ref="frmSearchPayments" v-on:submit.prevent class="space-y-8 divide-y divide-gray-200">
      <div v-if="isFilterPaymentsVisible" class="px-2 pt-2 border-b space-y-2">
        ...
        <div class="sm:col-span-6">
          <Dropdownlist name="search_payment_type" label="Payment Type" v-model="search_payment_type" @selected_item="handleSelectedForSearchIn_Payment_type" api="payment_type" />
        </div>
        <div class="sm:col-span-6">
          <label for="name" class="block text-sm font-medium text-gray-700"> Statement Period </label>
        ...
        <div class="sm:col-span-6">
          ...
          <div class="relative rounded-md shadow-sm">
            ...
            <input type="search" name="search_description" id="search_description" ref="search_description" class="focus:ring-green-500 focus:border-green-500 block w-full pl-10 sm:text-sm border-gray-300" placeholder="Search" />
          </div>
        </div>
        <div class="py-3 text-right">
          <button type="button" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">Reset</button>
          <button @click="searchPayments()" type="submit" class="bg-green-600 border border-transparent shadow-sm py-2 px-4 inline-flex justify-center text-sm font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">Search</button>
        </div>
      </div>
    </form>
  </div>
