...
<script>
    ...
    methods: {
        async upsertRole(args) {
            try {
                if (this.$_.isUndefined(this.data.id)) {
                    const {
                        data
                    } = await this.$axios.post(this.$config.apiURL  + "Role", this.data);
                } else {
                    const {
                        data
                    } = await this.$axios.put(this.$config.apiURL  + "Role/" + this.data.id, this.data);
                }
                this.data = {}
                this.isUpsertRoleVisible = !this.isUpsertRoleVisible;
                $nuxt.$emit('evtRefreshRoleDatatable')
        ...
        async deleteRole() {
            try {
                ...
                const {
                    data
                } = await this.$axios.delete(this.$config.apiURL  + "Role/" + this.data.id);
                $nuxt.$emit('evtRefreshRoleDatatable')
    ...
    created() {
        this.$nuxt.$on('evtUpsertRole', (data) => {
            if (data !== undefined) {
                this.data = data;
                this.isUpsertRoleVisible = !this.isUpsertRoleVisible;
                return this.data;
        ...
        this.$nuxt.$on('evtDeleteRole', (data) => {
            if (data !== undefined) {
                this.data = data;
                this.deleteRole();
            }
    ...
    beforeDestroy() {
        this.$nuxt.$off('evtUpsertRole');
        this.$nuxt.$off('evtDeleteRole');
    }
};
</script>