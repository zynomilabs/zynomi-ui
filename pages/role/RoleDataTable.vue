				...
				<div class="flex-1 m-1 p-1">
					...
					>
						...
						<v2-table-column label="" prop="" width="30">
							<template slot-scope="scope">
								<button
									@click="$nuxt.$emit('evtUpsertRole',scope.row)"
						...
						<v2-table-column label="" prop="" width="30">
							<template slot-scope="scope">
								<button
									v-if="!scope.row.mark_as_delete"
									@click="$nuxt.$emit('evtDeleteRole',scope.row)"
									class="inline-flex space-x-2 text-gray-400 hover:text-gray-500"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
...
<script>
	     ...
	     * RoleDataTable.vue
	     *
	     * @description :: Datatable / grid to list [Role] records
	    ...
	    methods: {
	        async getRole(page) {
	            try {
	                ...
	                let api = this.$config.apiURL  + "Role?sort=id%20DESC&limit=" + pageSize + "&skip=" + offset;
	    ...
	    created() {
	        ...
	        //3) Refresh table
	        this.$nuxt.$on('evtRefreshRoleDatatable', (data) => {
	            this.getRole(this.currentPage);
	        });
	        //4) Search
	        this.$nuxt.$on('evtSearchRole', (data) => {
