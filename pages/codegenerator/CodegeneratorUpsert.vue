<template>
    <section v-if="isUpsertCodegeneratorVisible" class="fixed inset-0 overflow-hidden" aria-labelledby="slide-over-title"
                ...
                <div class="w-screen max-w-md">
                    <div class="h-full divide-y divide-gray-200 flex flex-col bg-white shadow-xl">
                            ...
                            <header class="space-y-1 py-6 px-4 bg-green-700 sm:px-6">
                                <div class="flex items-center justify-between space-x-3">
                                    ...
                                    <div class="h-7 flex items-center">
                                        <button v-on:click="closeCodegeneratorPanel()" aria-label="Close panel"
                                ...
                                <div>
                                    <p class="text-sm leading-5 text-green-300">
                                        Create or update Codegenerator record
                                    </p>
                                </div>
                            </header>
                            <div class="flex-1 flex flex-col justify-between">
                                <div class="px-4 divide-y divide-gray-200 sm:px-6">
                                    <div class="space-y-6 pt-6 pb-5">
                                        <form name="frmCodegenerator" id="frmCodegenerator"
                                                ...
                                                <div class="bg-white space-y-6">
                                                    <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                                                        ...
                                                        <div class="sm:col-span-6">
                                                            ...
                                                            <div class="mt-1 rounded-md shadow-sm">
                                                                <input v-model="data.entity" type="text" id="entity"
                                                                    name="entity" autocomplete="entity"
                                                        ...
                                                        <div class="sm:col-span-6">
                                                            ...
                                                            <div class="mt-1 rounded-md shadow-sm">
                                                                <input v-model="data.inherit_base" type="text" id="inherit_base"
                                                                    name="inherit_base" autocomplete="inherit_base"
                                                                    class="flex-1 focus:ring-green-500 focus:border-green-500 block w-full min-w-0 sm:text-sm border-gray-300">
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="sm:col-span-6">
                                                            ...
                                                            <div class="mt-1 rounded-md shadow-sm">
                                                                <input v-model="data.generate_backend" type="text" id="generate_backend"
                                                                    name="generate_backend" autocomplete="generate_backend"
                                                                    class="flex-1 focus:ring-green-500 focus:border-green-500 block w-full min-w-0 sm:text-sm border-gray-300">
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="sm:col-span-6">
                                                            ...
                                                            <div class="mt-1 rounded-md shadow-sm">
                                                                <input v-model="data.generate_frontend" type="text" id="generate_frontend"
                                                                    name="generate_frontend" autocomplete="generate_frontend"
                                                                    class="flex-1 focus:ring-green-500 focus:border-green-500 block w-full min-w-0 sm:text-sm border-gray-300">
                                                            </div>
                                                        </div>
                        ...
                        <div class="w-full flex-shrink-0 px-4 py-4 space-x-4 flex justify-end">
                            ...
                            <span class="inline-flex rounded-md shadow-sm">
                                <button v-on:click="upsertCodegenerator" type="submit"
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                isUpsertCodegeneratorVisible: false,
                data: { },
                api: ""
        }
    },
    methods: {
        async upsertCodegenerator(args) {
            try {
                if (this.$_.isUndefined(this.data.id)) {
                    const {
                        data
                    } = await this.$axios.post(this.$config.apiURL  + "Codegenerator", this.data);
                } else {
                    const {
                        data
                    } = await this.$axios.put(this.$config.apiURL  + "Codegenerator/" + this.data.id, this.data);
                }
                this.data = {}
                this.isUpsertCodegeneratorVisible = !this.isUpsertCodegeneratorVisible;
                $nuxt.$emit('evtRefreshCodegeneratorDatatable')
        ...
        async deleteCodegenerator() {
            try {
                ...
                const {
                    data
                } = await this.$axios.delete(this.$config.apiURL  + "Codegenerator/" + this.data.id);
                $nuxt.$emit('evtRefreshCodegeneratorDatatable')
    ...
    created() {
        this.$nuxt.$on('evtUpsertCodegenerator', (data) => {
            if (data !== undefined) {
                this.data = data;
                this.isUpsertCodegeneratorVisible = !this.isUpsertCodegeneratorVisible;
                return this.data;
        ...
        //2) Delete Codegenerator record and then rebind datatable
        this.$nuxt.$on('evtDeleteCodegenerator', (data) => {
            if (data !== undefined) {
                this.data = data;
                this.deleteCodegenerator();
            }
        });
    },
    beforeDestroy() {
        this.$nuxt.$off('evtUpsertCodegenerator');
        this.$nuxt.$off('evtDeleteCodegenerator');
    }
};
</script>