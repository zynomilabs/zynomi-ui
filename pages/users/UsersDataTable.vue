    ...
    <div class="flex">
      <div class="flex-1 bg-gradient-to-r">
        <div class="flex-1 m-1 p-1">
          <v2-table :data="Users.data" :default-sort="{ prop: 'name', order: 'ascending' }" @sort-change="handleSortChange" :loading="loading" :total="total" :pagination-info="paginationInfo" :shown-pagination="true" @page-change="getUsers" border :row-class-name="getRowClassName" height="450" ref="mydt">
            ...
            <v2-table-column label="Phone" prop="phone">
              <template slot-scope="scope">
                <div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
            ...
            <v2-table-column label="" prop="" width="30">
              <template slot-scope="scope">
                <button @click="$nuxt.$emit('evtUpsertUsers', scope.row)" class="inline-flex space-x-2 text-gray-400 hover:text-gray-500">
            ...
            <v2-table-column label="" prop="" width="30">
              <template slot-scope="scope">
                <button v-if="!scope.row.mark_as_delete" @click="$nuxt.$emit('evtDeleteUsers', scope.row)" class="inline-flex space-x-2 text-gray-400 hover:text-gray-500">
 ...
 * UsersDataTable.vue
 *
 * @description :: Datatable / grid to list [Users] records
...
export default {
  ...
  methods: {
    async getUsers(page) {
      try {
        ...
        let api = this.$config.apiURL + 'Users?sort=id%20DESC&limit=' + pageSize + '&skip=' + offset;
  ...
  mounted() {},
  created() {
    //1) Bind Users Datatable when the view is mounted
    this.getUsers(this.currentPage);

    //2) After successful Insert or update or Delete of the record, rebind the datatable
    this.$nuxt.$on('evtBindUsersDatatable', (data) => {
      if (data !== undefined) {
    ...
    //3) Refresh table
    this.$nuxt.$on('evtRefreshUsersDatatable', (data) => {
      this.getUsers(this.currentPage);
    });
    //4) Search
    this.$nuxt.$on('evtSearchUsers', (data) => {
