        ...
        <div class="flex-1 m-1 p-1">
          <v2-table :data="Bot.data" :default-sort="{ prop: 'name', order: 'ascending' }" @sort-change="handleSortChange" :loading="loading" :total="total" :pagination-info="paginationInfo" :shown-pagination="true" @page-change="getBot" border :row-class-name="getRowClassName" height="450" ref="mydt">
            ...
            <v2-table-column label="" prop="" width="30">
              <template slot-scope="scope">
                <button @click="$nuxt.$emit('evtUpsertBot', scope.row)" class="inline-flex space-x-2 text-gray-400 hover:text-gray-500">
            ...
            <v2-table-column label="" prop="" width="30">
              <template slot-scope="scope">
                <button v-if="!scope.row.mark_as_delete" @click="$nuxt.$emit('evtDeleteBot', scope.row)" class="inline-flex space-x-2 text-gray-400 hover:text-gray-500">
 ...
 * BotDataTable.vue
 *
 * @description :: Datatable / grid to list [Bot] records
...
export default {
  ...
  methods: {
    async getBot(page) {
      try {
        ...
        let api = this.$config.apiURL + 'Bots?sort=id%20DESC&limit=' + pageSize + '&skip=' + offset;
        console.log(api);
  ...
  mounted() {},
  created() {
    //1) Bind Bot Datatable when the view is mounted
    this.getBot(this.currentPage);

    //2) After successful Insert or update or Delete of the record, rebind the datatable
    this.$nuxt.$on('evtBindBotDatatable', (data) => {
      if (data !== undefined) {
    ...
    //3) Refresh table
    this.$nuxt.$on('evtRefreshBotDatatable', (data) => {
      this.getBot(this.currentPage);
    });
    //4) Search
    this.$nuxt.$on('evtSearchBot', (data) => {
