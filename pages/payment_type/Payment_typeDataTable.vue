				...
				<div class="flex-1 m-1 p-1">
					...
					>
						...
						<v2-table-column label="" prop="" width="30">
							<template slot-scope="scope">
								<button
									@click="$nuxt.$emit('evtUpsertPayment_type',scope.row)"
						...
						<v2-table-column label="" prop="" width="30">
							<template slot-scope="scope">
								<button
									v-if="!scope.row.mark_as_delete"
									@click="$nuxt.$emit('evtDeletePayment_type',scope.row)"
									class="inline-flex space-x-2 text-gray-400 hover:text-gray-500"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
...
<script>
	    /**
	     * Payment_typeDataTable.vue
	     *
	     * @description :: Datatable / grid to list [Payment_type] records
	    ...
	    methods: {
	        async getPayment_type(page) {
	            try {
	                this.currentPage = page;
	                this.loading = false;
	                let offset = page == 1 ? 0 : (page - 1) * pageSize;
	                offset = isNaN(offset) ? 0 : offset;
	                let api = this.$config.apiURL  + "Payment_type?sort=id%20DESC&limit=" + pageSize + "&skip=" + offset;
	    ...
	    created() {
	        ...
	        //1) Bind Payment_type Datatable when the view is mounted
	        this.getPayment_type(this.currentPage);

	        //2) After successful Insert or update or Delete of the record, rebind the datatable
	        this.$nuxt.$on('evtBindPayment_typeDatatable', (data) => {
	            if (data !== undefined) {
	        ...
	        //3) Refresh table
	        this.$nuxt.$on('evtRefreshPayment_typeDatatable', (data) => {
	            this.getPayment_type(this.currentPage);
	        });
	        //4) Search
	        this.$nuxt.$on('evtSearchPayment_type', (data) => {
	            //alert(JSON.stringify(data))
	            this.searchCriteria = data;
	            this.getPayment_type(1);
	        });
	    },
	};
</script>
