    ...
    <div class="flex">
      <div class="flex-1 bg-gradient-to-r">
        <div class="flex-1 m-1 p-1">
          <v2-table :data="Processes.data" :default-sort="{ prop: 'name', order: 'ascending' }" @sort-change="handleSortChange" :loading="loading" :total="total" :pagination-info="paginationInfo" :shown-pagination="true" @page-change="getProcesses" border :row-class-name="getRowClassName" height="450" ref="mydt">
            ...
            <v2-table-column label="short_description" prop="short_description">
              <template slot-scope="scope">
                <div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
                  {{ scope.row.short_description | truncate(25) }}
                </div>
              </template>
            </v2-table-column>

            <v2-table-column label="requirements" prop="requirements">
              <template slot-scope="scope">
                <div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
                  {{ scope.row.requirements | truncate(25) }}
                </div>
              </template>
            </v2-table-column>
            ...
            <v2-table-column label="Status" prop="is_enabled" width="100">
              <template slot-scope="scope">
                <span v-if="!scope.row.mark_as_delete" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" :class="scope.row.is_enabled == true ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'">
            ...
            <v2-table-column label="" prop="" width="30">
              <template slot-scope="scope">
                <button @click="$nuxt.$emit('evtUpsertProcesses', scope.row)" class="inline-flex space-x-2 text-gray-400 hover:text-gray-500">
            ...
            <v2-table-column label="" prop="" width="30">
              <template slot-scope="scope">
                <button v-if="!scope.row.mark_as_delete" @click="$nuxt.$emit('evtDeleteProcesses', scope.row)" class="inline-flex space-x-2 text-gray-400 hover:text-gray-500">
...
<script>
/**
 * ProcessesDataTable.vue
 *
 * @description :: Datatable / grid to list [Processes] records
...
export default {
  ...
  methods: {
    async getProcesses(page) {
      try {
        ...
        let api = this.$config.apiURL + 'Processes?sort=id%20DESC&limit=' + pageSize + '&skip=' + offset;
  ...
  mounted() {},
  created() {
    //1) Bind Processes Datatable when the view is mounted
    this.getProcesses(this.currentPage);

    //2) After successful Insert or update or Delete of the record, rebind the datatable
    this.$nuxt.$on('evtBindProcessesDatatable', (data) => {
      if (data !== undefined) {
    ...
    //3) Refresh table
    this.$nuxt.$on('evtRefreshProcessesDatatable', (data) => {
      this.getProcesses(this.currentPage);
    });
    //4) Search
    this.$nuxt.$on('evtSearchProcesses', (data) => {
