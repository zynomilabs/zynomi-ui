        ...
        <div class="flex">
            <div class="flex-1 bg-gradient-to-r">
                <div class="flex-1 m-1 p-1">
                    <v2-table :data="Navigation.data" :default-sort='{prop: "name", order: "ascending"}'
                        @sort-change="handleSortChange" :loading="loading" :total="total"
                        :pagination-info="paginationInfo" :shown-pagination="true" @page-change="getNavigation" border
                        :row-class-name="getRowClassName" height="450" ref="mydt">
                        <v2-table-column type="selection" width="45">
                        </v2-table-column>
                        
                            <v2-table-column label="parent_code" prop="parent_code">
                                <template slot-scope="scope">
                                    <div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
                                        {{scope.row.parent_code | truncate(25)}} </div>
                                </template>
                            </v2-table-column>
                            ...
                            <v2-table-column label="name" prop="name">
                                <template slot-scope="scope">
                                    <div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
                                        {{scope.row.name | truncate(25)}} </div>
                                </template>
                            </v2-table-column>
                            ...
                            <v2-table-column label="description" prop="description">
                                <template slot-scope="scope">
                                    <div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
                                        {{scope.row.description | truncate(25)}} </div>
                                </template>
                            </v2-table-column>
                            
                            <v2-table-column label="icon" prop="icon">
                                <template slot-scope="scope">
                                    <div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
                                        {{scope.row.icon | truncate(25)}} </div>
                                </template>
                            </v2-table-column>
                            
                            <v2-table-column label="link" prop="link">
                                <template slot-scope="scope">
                                    <div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
                                        {{scope.row.link | truncate(25)}} </div>
                                </template>
                            </v2-table-column>
                            ...
                            <v2-table-column label="published_at" prop="published_at">
                                <template slot-scope="scope">
                                    <div class="ml-2 text-left text-xs font-medium text-gray-500 tracking-wider">
                                        {{scope.row.published_at | truncate(25)}} </div>
                                </template>
                            </v2-table-column>
                                ...
                                <v2-table-column label="" prop="" width="30">
                                    <template slot-scope="scope">
                                        <button @click="$nuxt.$emit('evtUpsertNavigation',scope.row)"
                                ...
                                <v2-table-column label="" prop="" width="30">
                                    <template slot-scope="scope">
                                        <button v-if="!scope.row.mark_as_delete"
                                            @click="$nuxt.$emit('evtDeleteNavigation',scope.row)"
                                            class="inline-flex space-x-2 text-gray-400 hover:text-gray-500">
                                            <svg xmlns="http://www.w3.org/2000/svg"
...
<script>
    /**
     * NavigationDataTable.vue
     *
     * @description :: Datatable / grid to list [Navigation] records
    ...
    methods: {
        async getNavigation(page) {
            try {
                ...
                let api = this.$config.apiURL + "Navigation?sort=id%20DESC&limit=" + pageSize + "&skip=" + offset;
                console.log(api)
    ...
    created() {
        ...
        //1) Bind Navigation Datatable when the view is mounted
        this.getNavigation(this.currentPage);

        //2) After successful Insert or update or Delete of the record, rebind the datatable
        this.$nuxt.$on('evtBindNavigationDatatable', (data) => {
            if (data !== undefined) {
        ...
        //3) Refresh table
        this.$nuxt.$on('evtRefreshNavigationDatatable', (data) => {
            this.getNavigation(this.currentPage);
        });
        //4) Search
        this.$nuxt.$on('evtSearchNavigation', (data) => {
